#!/usr/bin/env python3
"""Evaluate the already frozen GMDCSA24 inference exactly once (parser-fixed).
...
FALL_INTERVAL_MISSING_TO_PATTERN = re.compile(
    r"(?P<label>[^;\[\]]*?\bfall\w*(?:\s*\([^)]*\))?[^;\[\]]*)"
    r"\[\s*(?P<start>\d+(?:\.\d+)?)\s+"
    r"(?P<end>\d+(?:\.\d+)?)\s*\]",
    flags=re.IGNORECASE,
)
STANDALONE_FALL_LABEL_PATTERN = re.compile(
    r"(?:^|;)\s*(?P<label>[^;\[\]]*\bfall\w*"
    r"(?:\s*\([^)]*\))?[^;\[\]]*)\s*(?:;|$)",
    flags=re.IGNORECASE,
)
GENERIC_INTERVAL_PATTERN = re.compile(
    r"\[\s*(?P<start>\d+(?:\.\d+)?)\s*to\s*"
    r"(?P<end>\d+(?:\.\d+)?)\s*\]",
    flags=re.IGNORECASE,
)
...
def parse_fall_intervals(classes_text: str, actual_duration: float):
    intervals = []
    ...
    if intervals:
        return intervals, "official_explicit_interval"

    # Official anomaly: Subject 4/Fall/09 uses "[2.3 6]" and omits "to".
    for match in FALL_INTERVAL_MISSING_TO_PATTERN.finditer(classes_text or ""):
        start = float(match.group("start"))
        end = float(match.group("end"))
        label = " ".join(match.group("label").split())
        intervals.append(
    ...
    if intervals:
        return intervals, "repaired_missing_to_separator"

    # Official anomaly: Subject 4/Fall/15 names Falling (FW) without a range.
    # Its last labeled non-fall interval ends at 1.7 s. The deterministic
    # complement rule assigns the remainder of this Fall video to the named
    # fall class.
    standalone = STANDALONE_FALL_LABEL_PATTERN.search(classes_text or "")
    nonfall_intervals = list(GENERIC_INTERVAL_PATTERN.finditer(classes_text or ""))
    if standalone and nonfall_intervals and actual_duration > 0.0:
        onset = max(float(match.group("end")) for match in nonfall_intervals)
        label = " ".join(standalone.group("label").split())
        return [
            {
                "label": label,
                "fall_type": normalize_fall_type(label),
                "start_seconds": onset,
                "end_seconds": actual_duration,
            }
        ], "repaired_complement_after_last_nonfall_interval"
    return [], "unparsed"
...
def load_official_metadata() -> tuple:
    ...
    for mapping in mapping_rows:
        ...
        intervals, interval_source = parse_fall_intervals(
            classes_text, actual_duration
        )
        ...
        row = {
            ...
            "fall_type": fall_type,
            "fall_interval_source": interval_source,
    ...
    try:
        ...
        summary = {
            ...
            "dataset": {
                ...
                "fall_interval_source_counts": dict(
                    sorted(
                        Counter(
                            info["fall_interval_source"]
                            for info in metadata.values()
                            if info["category"] == "Fall"
                        ).items()
                    )
